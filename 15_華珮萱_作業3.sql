...
-- 整數)
select ename, ROUND(((datediff(curdate(),hiredate))/30),0) "MONTHS_WORKED"
from emp;

-- 6. 顯示如下格式：<員工姓名> earns <薪水> monthly but wants <3倍的薪水>.並將表頭顯示為Dream 
-- Salaries。
Select concat( ename,' earns ', sal , ' monthy but wants ', sal*3) "Dream Salaries"
from emp;
...
select concat(ename, repeat("*",sal/100)) "EMPLOYEE_AND_THEIR_SALARIES"
from emp
order by sal desc;